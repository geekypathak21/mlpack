...
class GAN
{
 public:
  ...
  GAN(Model generator,
      ...
      const typename MatType::elem_type clippingParameter = 0.01,
      const typename MatType::elem_type lambda = 10.0);
  ...
  GAN(GAN&& other);
  //! Copy operator.
  GAN& operator=(const GAN& other);
  //! Move assignment operator.
  GAN& operator=(GAN&& other);
   ...
   */
  void ResetData(MatType trainData);
  ...
  typename MatType::elem_type Train(MatType trainData,
   ...
   */
  template<typename Policy = PolicyType>
  std::enable_if_t<std::is_same_v<Policy, WGAN>, typename MatType::elem_type>
  Evaluate(const MatType& parameters,
           const size_t i,
           const size_t batchSize);
   ...
   */
  void Predict(MatType input, MatType& output);

  //! Return the parameters of the network.
  const MatType& Parameters() const { return parameter; }
  //! Modify the parameters of the network.
  MatType& Parameters() { return parameter; }
  ...
  MatType& Responses() { return responses; }

  //! Get the matrix of data points (predictors).
  const MatType& Predictors() const { return predictors; }
  //! Modify the matrix of data points (predictors).
  MatType& Predictors() { return predictors; }
